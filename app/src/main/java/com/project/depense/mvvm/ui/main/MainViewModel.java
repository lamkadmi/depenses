...
public class MainViewModel extends BaseViewModel<MainNavigator> {
    ...
    private final MutableLiveData<List<ObjectifItemViewModel>> objectifItemsLiveData;
    ...
    public MainViewModel(DataManager dataManager, SchedulerProvider schedulerProvider) {
        ...
        objectifItemsLiveData = new MutableLiveData<>();
        loadQuestionCards();
        fetchObjectifs();
    }
    ...
    public MutableLiveData<List<ObjectifItemViewModel>> getObjectifItemsLiveData() {
        return objectifItemsLiveData;
    }
    ...
    public void fetchObjectifs() {
        setIsLoading(true);
        getCompositeDisposable().add(getDataManager()
                .getObjectifsData()
                .map(openSourceResponse -> openSourceResponse)
                .flatMap(this::getViewModelList)
                .subscribeOn(getSchedulerProvider().io())
                .observeOn(getSchedulerProvider().ui())
                .subscribe(objectifs -> {
                    objectifItemsLiveData.setValue(objectifs);
                    setIsLoading(false);
                }, throwable -> {
                    setIsLoading(false);
    ...
    private Single<List<ObjectifItemViewModel>> getViewModelList(List<QuestionCardData> repoList) {
        return Observable.fromIterable(repoList)
                .map(repo -> new ObjectifItemViewModel(
                        repo.question.createdAt, repo.question.questionText)).toList();
    }
